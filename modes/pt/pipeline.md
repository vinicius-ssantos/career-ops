...
> Recomendacao operacional: usar LinkedIn como fonte principal de descoberta e usar este modo para processar as vagas priorizadas. `scan` fica como fonte secundaria.
...
> Nota: Os titulos das secoes podem estar em EN ("Pending"/"Processed"), ES ("Pendientes"/"Procesadas"), DE ("Offen"/"Verarbeitet") ou PT-BR ("Pendentes"/"Processadas"). Ao ler, ser flexivel; ao escrever, manter o estilo do arquivo existente.

## Deteccao inteligente de JD a partir da URL

1. **Playwright (preferido):** `browser_navigate` + `browser_snapshot`. Funciona com SPAs.
...
**Casos especiais:**
- **LinkedIn**: Pode exigir login -> preferir salvar o JD em `jds/...` e processar `local:jds/...`; se nao houver JD local, marcar com `[!]` e pedir ao candidato para colar o texto
...
- **Gupy / Greenhouse / Lever**: Plataformas comuns no Brasil; Playwright costuma funcionar bem
- **Vagas.com.br / InfoJobs / Catho**: Portais brasileiros, geralmente acessiveis via WebFetch
- **LinkedIn BR**: Mesmas restricoes do LinkedIn global; usar JD local quando a extracao direta falhar
